...
function main()
    ...
    mask_c_inset_init = isfinite.(λs) .&
                    isfinite.(init_hess_mineig) .&
                    isfinite.(init_hess_mineig_err) .&
                    (λs .>= 0.0) .&
                    (λs .<= 1.0)

mask_c_inset_final = isfinite.(λs) .&
                     isfinite.(final_hess_mineig) .&
                     isfinite.(final_hess_mineig_err) .&
                     (λs .>= 0.0) .&
                     (λs .<= 1.0)
    ...
    any(mask_c_inset_init) || error("No usable positive entries for panel (c) inset init curve.")
    any(mask_c_inset_final) || error("No usable positive entries for panel (c) inset final curve.")
    ...
    # ------------------------------------------------------------
    # (c) init gradient + init/final Hessian inset
    # ------------------------------------------------------------
    axc = styled_axis(fig[2, 1];
        xlabel = L"\lambda",
        ylabel = L"||\mathbf{g}_{\mathrm{init}}|| ",
        xticks = xticks_to1,
    )
    ...
    axc_inset = Axis(fig[2, 1];
        ...
        xticks = (0.0:0.5:1.0, [@sprintf("%.1f", x) for x in 0.0:0.5:1.0]),
        xticklabelsize = INSET_TICKSIZE,
        yticklabelsize = INSET_TICKSIZE,
        xlabelsize = INSET_LABELSIZE,
        ylabelsize = INSET_LABELSIZE,
    ...
    errorbars!(axc_inset,
        λs[mask_c_inset_init],
        init_hess_mineig[mask_c_inset_init],
        init_hess_mineig_err[mask_c_inset_init];
        whiskerwidth = 0
    )
    ...
    errorbars!(axc_inset,
        λs[mask_c_inset_final],
        final_hess_mineig[mask_c_inset_final],
        final_hess_mineig_err[mask_c_inset_final];
        whiskerwidth = 0
    )
    ...
    out_png = joinpath(
        PLOTS_DIR,
        "qiigs_abcd_2x2_v9_initgrad_dualhess_spincollapsed_unweighted_N$(N)_k$(k)_ngraphs$(ngraphs)_ninit$(n_inits)_abin$(abin_tag)_mainconv$(conv_main_tag).png"
    )
