# scripts/qiigs_lambda_sweep_unique_minima_optcurv.jl
#
# Adds:
...
#   - ΔdevTheta_abs:              forward difference in devTheta_abs_mean across λ values
#
# NOTE:
...
#
# OPTIMUM-CURVATURE MODE:
#   - If compute_optimum_curvature=false:
#       old behavior (no optimum-curvature diagnostics)
#   - If compute_optimum_curvature=true:
#       solver computes curvature diagnostics only when the rounded final energy
#       matches the known global optimum energy for that graph
#   - Curvature is evaluated at the continuous pre-rounding angles θ_best
#   - If the optimum is not reached for a run, optcurv_* fields are NaN/false/0
#   - In aggregation, NaNs do NOT contribute
#
# IMPORTANT:
#   - angle_conv is included in the FINAL output filename
#   - Hessian mode/tolerance are also included in the filename
#   - optimum-curvature mode/tolerance are also included in the filename
...
function main()
    println("====================================================================================")
    println("  QiIGS λ-sweep: uniques + ratio(mean/best) + success + grad + dev + optimum curv ")
    ...
    # Generic Hessian options (old mode)
    compute_hessian = false
    hessian_tol = 1e-8

    # New: optimum-curvature mode
    compute_optimum_curvature = true
    optimal_energy_atol = 1e-9

    ckpt_namespace = "v9_optcurv"
    ...
    ckpt_path = joinpath(
        save_dir,
        "checkpoint_unique_ratio_meanbest_succ_grad_optcurv_$(tag)_$(init_sig)_ninit$(n_inits)_outer$(iterations)_inner$(inner_iterations)_thr$(replace(@sprintf("%.3f", success_thr), "."=>"p")).json"
    )
    ...
    @info "compute_optimum_curvature" compute_optimum_curvature
    @info "optimal_energy_atol" optimal_energy_atol
    ...
    # Old Hessian-derived accumulators
    ...
    # New optimum-curvature accumulators (NaN-safe via explicit counts)
    ...
    sum_optcurv_hess_mineig_mean  = zeros(Float64, nλ)
    sum_optcurv_hess_mineig_mean2 = zeros(Float64, nλ)
    n_optcurv_hess_mineig_graphs  = zeros(Int, nλ)
    ...
    if isfile(ckpt_path)
        ...
        if ck["N"] == N && ck["k"] == k && ck["weighted_flag"] == weighted &&
           ...
           isapprox(Float64(get(ck, "optimal_energy_atol", optimal_energy_atol)), optimal_energy_atol; atol=0.0, rtol=0.0)
            ...
            if haskey(ck, "sum_optcurv_reached_frac")
                ...
                sum_optcurv_hess_mineig_mean  .= Float64.(ck["sum_optcurv_hess_mineig_mean"])
                sum_optcurv_hess_mineig_mean2 .= Float64.(ck["sum_optcurv_hess_mineig_mean2"])
                n_optcurv_hess_mineig_graphs  .= Int.(ck["n_optcurv_hess_mineig_graphs"])
            end
    ...
    for idx in start_index:length(graph_seeds)
        ...
        devTheta_abs_delta = fill(NaN, nλ)
        ...
        optcurv_hess_mineig_mean = fill(NaN, nλ)
        ...
        for (iλ, λ) in enumerate(λs)
            ...
            # New optimum-curvature counters per λ
            n_optcurv_reached = 0
            n_optcurv_minpos = 0
            n_optcurv_condpos = 0
            n_optcurv_mineig = 0
            ...
            for r in 1:n_inits
                ...
                if compute_optimum_curvature
                    ...
                    if reached
                        ...
                        v_mineig = Float64(get(sol.metadata, :optcurv_hess_mineig, NaN))
                        ...
                        if isfinite(v_mineig)
                            sum_optcurv_mineig += v_mineig
                            n_optcurv_mineig += 1
                        end
                    end
                end
            ...
            if iλ > 1
                devTheta_abs_delta[iλ] = devTheta_abs_mean[iλ] - devTheta_abs_mean[iλ - 1]
            end
            ...
            if compute_optimum_curvature
                ...
                optcurv_hess_mineig_mean[iλ] = (n_optcurv_mineig > 0) ? (sum_optcurv_mineig / n_optcurv_mineig) : NaN
            end
            ...
            if compute_optimum_curvature
                @printf("    λ=%.4f  uniques(spin)=%d  angles_raw=%d  angles_reduced=%d  psucc=%.4f  devTheta=%.6f  ΔdevTheta=%s  optcurv_hit=%.4f  optcurv_minpos=%s  optcurv_condpos=%s\n",
                    λ,
                    Int(unique_count[iλ]),
                    ...
                    isfinite(devTheta_abs_delta[iλ]) ? @sprintf("%.6e", devTheta_abs_delta[iλ]) : "NaN",
                    optcurv_reached_frac[iλ],
                    isfinite(optcurv_hess_minpos_mean[iλ]) ? @sprintf("%.6e", optcurv_hess_minpos_mean[iλ]) : "NaN",
                    isfinite(optcurv_hess_cond_pos_mean[iλ]) ? @sprintf("%.6e", optcurv_hess_cond_pos_mean[iλ]) : "NaN")
            elseif compute_hessian
                @printf("    λ=%.4f  uniques(spin)=%d  angle_min_raw=%d  angle_min_red=%d  frac_min=%.4f  frac_saddle=%.4f  devZ=%.4f  devTheta=%.4f\n",
                    λ,
                    Int(unique_count[iλ]),
                    Int(unique_angle_count_raw[iλ]),
                    Int(unique_angle_count_reduced[iλ]),
                ...
                @printf("    λ=%.4f  uniques(spin)=%d  angles_raw=%d  angles_reduced=%d  devZ=%.4f  devTheta=%.4f\n",
                    λ,
                    Int(unique_count[iλ]),
                    Int(unique_angle_count_raw[iλ]),
                    Int(unique_angle_count_reduced[iλ]),
        ...
        for iλ in 1:nλ
            ...
            if compute_optimum_curvature
                ocf = optcurv_reached_frac[iλ]
                if isfinite(ocf)
                    sum_optcurv_reached_frac[iλ]  += ocf
                    sum_optcurv_reached_frac2[iλ] += ocf^2
                end

                ocminpos = optcurv_hess_minpos_mean[iλ]
                if isfinite(ocminpos)
                    sum_optcurv_hess_minpos_mean[iλ]  += ocminpos
                    sum_optcurv_hess_minpos_mean2[iλ] += ocminpos^2
                    n_optcurv_hess_minpos_graphs[iλ] += 1
                end

                occondpos = optcurv_hess_cond_pos_mean[iλ]
                if isfinite(occondpos)
                    sum_optcurv_hess_cond_pos_mean[iλ]  += occondpos
                    sum_optcurv_hess_cond_pos_mean2[iλ] += occondpos^2
                    n_optcurv_hess_cond_pos_graphs[iλ] += 1
                end

                ocmineig = optcurv_hess_mineig_mean[iλ]
                if isfinite(ocmineig)
                    sum_optcurv_hess_mineig_mean[iλ]  += ocmineig
                    sum_optcurv_hess_mineig_mean2[iλ] += ocmineig^2
                    n_optcurv_hess_mineig_graphs[iλ] += 1
                end
            end
        ...
        ckpt = Dict(
            ...
            "sum_optcurv_hess_mineig_mean" => sum_optcurv_hess_mineig_mean,
            "sum_optcurv_hess_mineig_mean2" => sum_optcurv_hess_mineig_mean2,
            "n_optcurv_hess_mineig_graphs" => n_optcurv_hess_mineig_graphs,
    ...
    for (iλ, λ) in enumerate(λs)
        ...
        optcurv_hess_mineig_mean_mean = "none"
        optcurv_hess_mineig_mean_stderr = "none"
        ...
        if compute_optimum_curvature
            ...
            if n_optcurv_hess_minpos_graphs[iλ] > 0
                μocmp, eocmp = mean_and_stderr(
                    sum_optcurv_hess_minpos_mean[iλ],
                    sum_optcurv_hess_minpos_mean2[iλ],
                    n_optcurv_hess_minpos_graphs[iλ],
                )
                optcurv_hess_minpos_mean_mean = μocmp
                optcurv_hess_minpos_mean_stderr = eocmp
            end

            if n_optcurv_hess_cond_pos_graphs[iλ] > 0
                μoccp, eoccp = mean_and_stderr(
                    sum_optcurv_hess_cond_pos_mean[iλ],
                    sum_optcurv_hess_cond_pos_mean2[iλ],
                    n_optcurv_hess_cond_pos_graphs[iλ],
                )
                optcurv_hess_cond_pos_mean_mean = μoccp
                optcurv_hess_cond_pos_mean_stderr = eoccp
            end

            if n_optcurv_hess_mineig_graphs[iλ] > 0
                μocmg, eocmg = mean_and_stderr(
                    sum_optcurv_hess_mineig_mean[iλ],
                    sum_optcurv_hess_mineig_mean2[iλ],
                    n_optcurv_hess_mineig_graphs[iλ],
                )
                optcurv_hess_mineig_mean_mean = μocmg
                optcurv_hess_mineig_mean_stderr = eocmg
            end
        end
        ...
        results[iλ] = Dict(
            ...
            "n_optcurv_hess_minpos_graphs" => n_optcurv_hess_minpos_graphs[iλ],
            ...
            "n_optcurv_hess_cond_pos_graphs" => n_optcurv_hess_cond_pos_graphs[iλ],

            "optcurv_hess_mineig_mean_mean" => optcurv_hess_mineig_mean_mean,
            "optcurv_hess_mineig_mean_stderr" => optcurv_hess_mineig_mean_stderr,
            "n_optcurv_hess_mineig_graphs" => n_optcurv_hess_mineig_graphs[iλ],
        )
    end

    save_data = Dict(
        "experiment" => "qiigs_lambda_sweep_unique_minima_optcurv",
        "N" => N,
        "k" => k,
        "weighted_flag" => weighted,
        "graph_seeds" => graph_seeds,
        ...
        "solver" => Dict(
            ...
            "outputs" => begin
                ...
                if compute_hessian
                    append!(outs, [
                        "hess_mineig", "hess_maxeig", "hess_cond",
                        "hess_is_minimum", "hess_is_saddle",
                        "hess_is_maximum", "hess_is_degenerate"
                    ])
                end
                if compute_optimum_curvature
                    append!(outs, [
                        "optimal_energy_reached",
                        "optcurv_hess_mineig", "optcurv_hess_maxeig",
                        "optcurv_hess_minpos", "optcurv_hess_cond", "optcurv_hess_cond_pos",
                        "optcurv_hess_is_minimum", "optcurv_hess_is_saddle",
                        "optcurv_hess_is_maximum", "optcurv_hess_is_degenerate"
                    ])
                end
                outs
            end,
        ...
        "angle_space_diagnostics" => Dict(
            ...
            "delta_print_only" => "ΔdevTheta_abs = devTheta_abs_mean(λ_i) - devTheta_abs_mean(λ_{i-1})",
        ),

        "optimum_curvature_diagnostics" => Dict(
            "enabled" => compute_optimum_curvature,
            "source_angles" => "continuous pre-rounding θ_best",
            "selection_rule" => "diagnostics recorded only if rounded final energy matches known global optimum energy",
            "selection_energy" => "E_opt = sum_{i<j} w_ij - 2 * optimal_cut",
            "nan_policy" => "NaN values do not contribute to graph-averaged curvature summaries",
            "reported" => [
                "optcurv_reached_frac_mean",
                "optcurv_hess_minpos_mean_mean",
                "optcurv_hess_cond_pos_mean_mean",
                "optcurv_hess_mineig_mean_mean"
            ],
        ),
    ...
    out_path = joinpath(
        save_dir,
        "qiigs_unique_ratio_meanbest_succ_grad_optcurv_$(tag)_conv$(conv_tag)_abin$(abin_tag)_$(hmode_tag)_htol$(htol_tag)_$(optcurv_tag)_oeatol$(optetol_tag)_ngraphs$(n_graphs)_ninit$(n_inits)_outer$(iterations)_inner$(inner_iterations)_thr$(replace(@sprintf("%.3f", success_thr), "."=>"p")).json"
    )
