...
#   - For single-seed runs, outputs are saved under ..._seedX_...
#   - For multi-seed runs, outputs are saved under ..._graphsN_...
...
# robust comparison for graph seed lists loaded from JSON
same_graph_seeds(a, b) = collect(Int, a) == collect(Int, b)
...
function main()
    ...
    seed_tag = length(graph_seeds) == 1 ?
        "seed$(only(graph_seeds))" :
        "graphs$(length(graph_seeds))"

    save_dir = joinpath(
        RESULTS_DIR,
        "qiigs_unique_minima_N$(N)_k$(k)_$(seed_tag)_$(wtag)"
    )
    mkpath(save_dir)
    ...
    ckpt_path = joinpath(
        save_dir,
        "checkpoint_unique_ratio_meanbest_succ_grad_$(seed_tag)_$(tag)_$(init_sig)_ninit$(n_inits)_outer$(iterations)_inner$(inner_iterations)_thr$(replace(@sprintf("%.3f", success_thr), "."=>"p")).json"
    )
    ...
    if isfile(ckpt_path)
        ...
        if ck["N"] == N && ck["k"] == k && ck["weighted_flag"] == weighted &&
           same_graph_seeds(ck["graph_seeds"], graph_seeds) &&
    ...
    out_path = joinpath(
        save_dir,
        "qiigs_unique_ratio_meanbest_succ_grad_$(seed_tag)_$(tag)_conv$(conv_tag)_abin$(abin_tag)_$(hmode_tag)_htol$(htol_tag)_ngraphs$(n_graphs)_ninit$(n_inits)_outer$(iterations)_inner$(inner_iterations)_thr$(replace(@sprintf("%.3f", success_thr), "."=>"p")).json"
    )
