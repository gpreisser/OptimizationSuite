...
#
# NEW:
#   - writes a lightweight per-graph CSV with ONLY the runs that end in an
#     optimal minimum, defined by:
#         is_minimum && isfinite(ratio) && abs(ratio - 1.0) <= opt_tol
#
# NOTE:
...
function main()
    ...
    @info "Optimal-minimum CSV dir" optmin_dir
    ...
    for idx in start_index:length(graph_seeds)
        ...
        optmin_csv_path = joinpath(
            optmin_dir,
            "optimal_minima_hesscond_N$(N)_k$(k)_gseed$(gs)_$(wtag)_tol$(opttol_tag).csv"
        )

        open(optmin_csv_path, "w") do io_optmin
            println(io_optmin, "graph_seed,lambda,run_index,run_seed,hess_cond,hess_mineig,ratio")
            ...
            for (iλ, λ) in enumerate(λs)
                ...
                # Final-state Hessian counters per λ
                n_min = 0
                n_saddle = 0
                n_max = 0
                n_deg = 0
                ...
                for r in 1:n_inits
                    ...
                    if opt !== nothing
                        ...
                        is_opt = isfinite(ratio) && abs(ratio - 1.0) <= opt_tol

                        if compute_hessian && is_minimum && is_opt
                            hcond = Float64(get(sol.metadata, :hess_cond, NaN))
                            hmig = Float64(get(sol.metadata, :hess_mineig, NaN))

                            if isfinite(hcond)
                                @printf(
                                    io_optmin,
                                    "%d,%.12f,%d,%d,%.17g,%.17g,%.17g\n",
                                    gs, λ, r, run_seed, hcond, hmig, ratio
                                )
                            end
                        end
                    end
                ...
                if compute_hessian
                    @printf("    λ=%.4f  uniques(spin)=%d  angle_min_raw=%d  angle_min_red=%d  frac_min=%.4f  frac_saddle=%.4f  devZ=%.4f  devTheta=%.4f  init_hmineig=%.4e  n_init_hcond=%d\n",
                        λ,
                        Int(unique_count[iλ]),
    ...
    save_data = Dict(
        ...
        "optimal_minimum_records" => Dict(
            "condition" => "compute_hessian && hess_is_minimum && isfinite(ratio) && abs(ratio - 1.0) <= opt_tol",
            "opt_tol" => opt_tol,
            "per_graph_csv_dir" => optmin_dir,
            "columns" => ["graph_seed", "lambda", "run_index", "run_seed", "hess_cond", "hess_mineig", "ratio"],
        ),
    ...
    println("✔ Optimal-minimum CSVs saved under: $optmin_dir")
    println("Done.")
end
