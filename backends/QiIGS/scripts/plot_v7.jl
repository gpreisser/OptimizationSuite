...
function plot_panel_ab(data)
    ...
    linewidth = 2
        ...
        if data.has_kappa_cloud
            ...
            mask_κopt = isfinite.(data.λs) .&
                        isfinite.(data.κopt_median) .&
                        (data.κopt_median .> 0)

            errorbars!(axb_inset,
                data.λs[mask_κopt], data.κopt_median[mask_κopt], zeros(count(mask_κopt));
                color = Makie.wong_colors()[1],
                whiskerwidth = 0
            )

            scatterlines!(axb_inset,
                data.λs[mask_κopt], data.κopt_median[mask_κopt];
                color = Makie.wong_colors()[1],
                marker = :rect,
                markersize = 5,
                linewidth = 1.5
...
ps_color = Makie.wong_colors()[1]   # blue
g_color  = Makie.wong_colors()[2]   # orange
...
# right axis — must match SAME width + halign
...
#Legend(fig[3, 1:2],
 #   [
  #      [MarkerElement(color = Makie.wong_colors()[1], marker = :circle, markersize = 8),
   #      LineElement(color = Makie.wong_colors()[1], linewidth = 1.8)],
    #    [MarkerElement(color = Makie.wong_colors()[2], marker = :rect, markersize = 7),
     #    LineElement(color = Makie.wong_colors()[2], linewidth = 1.8)],
    #],
    #[L"P_{\mathrm{solved}}", L"\left| \mathbf{g} \right|"];
    #labelsize = 14,
