# scripts/plot_small_lambda_rawdelta_meanratio.jl
...
function load_mean_ratio_curve(path::AbstractString; λmax=0.25)
    data = JSON.parsefile(path)
    λs = Float64[]
    mean_ratio = Float64[]
    ...
    for entry in data["results_per_lambda"]
        ...
        if isfinite(λ) && isfinite(mr) && λ <= λmax
            push!(λs, λ)
            push!(mean_ratio, mr)
        end
    end
...
function raw_mean_abs_delta_curve(
    ...
    for λ in λs
        ...
        @printf("λ=%.4f   raw_mean|θ-π/4| = %.6e   n_kept=%d\n",
            Float64(λ), out[Float64(λ)], n_kept)
    end
...
function main()
    ...
    # fresh recomputation only for raw delta panel
    n_inits_raw = 2000
    iterations = 1
    ...
    @info "Using aggregated file" path

    λs, mean_ratio = load_mean_ratio_curve(path; λmax=λmax)
    isempty(λs) && error("No usable λ values found.")
    ...
    rawδ_dict = raw_mean_abs_delta_curve(
        W, N;
        λs=λs,
        n_inits=n_inits_raw,
    ...
    rawδ = [get(rawδ_dict, λ, NaN) for λ in λs]

    xticks_small = (0.0:0.05:0.25, [@sprintf("%.2f", x) for x in 0.0:0.05:0.25])
    ...
    axa = styled_axis(fig[1, 1];
        xlabel = L"\lambda",
        ylabel = L"\left\langle |\theta-\frac{\pi}{4}| \right\rangle_{\mathrm{raw}}",
        xticks = xticks_small,
    )

    mask_a = isfinite.(λs) .& isfinite.(rawδ)
    scatterlines!(axa,
        λs[mask_a], rawδ[mask_a];
        marker = :circle,
        markersize = 8
    )
    ...
    mask_b = isfinite.(λs) .& isfinite.(mean_ratio)
    scatterlines!(axb,
        λs[mask_b], mean_ratio[mask_b];
        marker = :circle,
        markersize = 8
    )
    ...
    text!(fig.scene, "(b)"; position=(0.53, 0.93), space=:relative, fontsize=18)
    ...
    out_png = joinpath(
        PLOTS_DIR,
        "small_lambda_rawdelta_meanratio_N$(N)_k$(k)_ngraphs$(ngraphs)_abin$(abin_tag)_conv$(conv_tag).png"
    )
