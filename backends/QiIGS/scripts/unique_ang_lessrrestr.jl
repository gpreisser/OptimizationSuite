...
function newest_final_qiigs(; N::Int=50, k::Int=3, weighted::Bool=false,
                             n_inits::Int=100, angle_bin::Float64=1e-1)
...
function main()
    ...
    angle_bin = 2e-2   # <-- change this to plot other datasets

    path = newest_final_qiigs(; N=N, k=k, weighted=weighted,
                               n_inits=n_inits, angle_bin=angle_bin)

    path === nothing && error("No FINAL QiIGS JSON found.")
...
out_png = joinpath(
    PLOTS_DIR,
    "qiigs_abcd_N$(N)_k$(k)_ninit$(n_inits)_abin$(abin_tag)_$(weighted ? "weighted" : "unweighted").png"
)
...
println(uniques_round[end])
println(uniques_angle[end])
    display(fig)
end
