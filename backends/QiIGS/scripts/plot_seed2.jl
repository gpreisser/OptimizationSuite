# scripts/plot_conor_seed2_unweighted_N50_multiconv_panelc.jl
...
function seed_tag_from_inputs(; graph_seed::Union{Nothing,Int}=nothing, ngraphs::Union{Nothing,Int}=nothing)
    if graph_seed !== nothing
        return "seed$(graph_seed)"
    elseif ngraphs !== nothing
        return "graphs$(ngraphs)"
    else
        error("Provide either graph_seed or ngraphs")
    end
end

function result_subdir(; N, k, weighted, graph_seed::Union{Nothing,Int}=nothing, ngraphs::Union{Nothing,Int}=nothing)
    wtag = weighted ? "weighted" : "unweighted"
    stag = seed_tag_from_inputs(; graph_seed=graph_seed, ngraphs=ngraphs)
    return joinpath(
        RESULTS_DIR,
        "qiigs_unique_minima_N$(N)_k$(k)_$(stag)_$(wtag)"
    )
end

function list_matching_jsons(; N, k, n_inits, angle_bin, weighted=false,
                              graph_seed::Union{Nothing,Int}=nothing,
                              ngraphs::Union{Nothing,Int}=nothing,
                              require_hessian=true)
    abin_tag = replace(@sprintf("%.3g", angle_bin), "." => "p")
    stag = seed_tag_from_inputs(; graph_seed=graph_seed, ngraphs=ngraphs)

    subdir = result_subdir(;
        N=N, k=k, weighted=weighted,
        graph_seed=graph_seed, ngraphs=ngraphs
    )
    isdir(subdir) || error("Directory not found: $subdir")

    files = readdir(subdir; join=true)

    prefix = "qiigs_unique_ratio_meanbest_succ_grad_$(stag)_lam0p000_to_1p000_d"
    abin_piece = "_abin$(abin_tag)_"
    ninit_piece = "_ninit$(n_inits)_outer1_inner5000_thr0p999.json"
...
function newest_matching_json(; N, k, n_inits, angle_bin, angle_conv, weighted=false,
                               graph_seed::Union{Nothing,Int}=nothing,
                               ngraphs::Union{Nothing,Int}=nothing,
                               require_hessian=true)
...
function main()
    ...
    any(mask_gnm) || error("No usable entries for panel (d).")
    ...
    out_png = joinpath(
        PLOTS_DIR,
        "qiigs_acbd_hessianb_seed$(graph_seed)_unweighted_N$(N)_k$(k)_ninit$(n_inits)_abin$(abin_tag)_mainconv$(conv_main_tag)_panelc_multiconv.png"
    )
