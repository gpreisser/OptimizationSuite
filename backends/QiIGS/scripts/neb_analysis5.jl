...
function main()
    ...
    # choose which reduced minima to connect
    min_idx1 = 1
    min_idx2 = 4
    pair_tag = "pair$(min_idx1)_$(min_idx2)"
    ...
    png_path = joinpath(
        PLOTS_DIR,
        "neb_relaxed_vs_straight_clean_N$(N)_k$(k)_seed$(graph_seed)_$(wtag)_lam$(λtag)_$(pair_tag)_nimg$(n_images).png"
    )

    json_path = joinpath(
        NEB_DIR,
        "neb_relaxed_vs_straight_clean_N$(N)_k$(k)_seed$(graph_seed)_$(wtag)_lam$(λtag)_$(pair_tag)_nimg$(n_images).json"
    )
    ...
    save_data = Dict(
        ...
        "lambda" => λtarget,
        "pair_indices" => [min_idx1, min_idx2],
        "pair_tag" => pair_tag,
        "mins_path" => mins_path,
        "theta_distance_modpi" => theta_distance_modpi(θ1, θ2),
        "endpoint_rounded_energies" => [
            Float64(red[min_idx1]["rounded_energy"]),
            Float64(red[min_idx2]["rounded_energy"]),
        ],
        "neb_parameters" => Dict(
            "n_images" => n_images,
            "spring_k" => spring_k,
    ...
    ax1 = Axis(fig[1, 1];
        ...
        title = "λ = $(λtarget), pair $(min_idx1)↔$(min_idx2)",
        xgridvisible = false,
        ygridvisible = false,
        xtickalign = 1,
        ytickalign = 1,
    ...
    Label(
        fig[0, 1],
        @sprintf(
            "graph seed = %d   pair = %d↔%d   straight barrier = %.6f   relaxed barrier = %.6f",
            graph_seed, min_idx1, min_idx2, barrier_straight, barrier_relaxed
        ),
        fontsize = 18
    )
