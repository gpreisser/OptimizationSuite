...
# (a) mean_ratio_mean vs unique_count_mean (scatter, colored by λ)
# (b) unique_count_mean vs λ
...
function main()
    ...
    mask_a = isfinite.(uniques) .& isfinite.(mean_ratio)
    ...
    # ------------------------------------------------------------
    # (a) λ vs N_unique — scatter colored by mean_ratio (r)
    # ------------------------------------------------------------
    suba = GridLayout()
    fig[1, 1] = suba
    ...
    # ------------------------------------------------------------
    # (b) (N_unique / n_inits) vs λ
    # ------------------------------------------------------------
    axb = styled_axis(fig[1, 2];
        xlabel = L"\lambda",
        ylabel = L"\langle N_{\mathrm{unique}}\rangle / n_{\mathrm{init}}",
        xλ = true
    )

    uniques_norm = uniques ./ n_inits
    mask_b = isfinite.(uniques_norm) .& isfinite.(λs)

    scatterlines!(axb, λs[mask_b], uniques_norm[mask_b]; markersize=8)
    ylims!(axb, 0.0, 1.0)   # force 0..1 since normalized
