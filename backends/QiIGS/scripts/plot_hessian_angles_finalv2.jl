...
function load_curves(json_path::AbstractString)
    ...
    for entry in rpl
        ...
        gnme   = getnum(entry, "grad_norm_meaninner_stderr")
...
function main()
    ...
    mask_c = isfinite.(λs) .&
             isfinite.(gn_meaninner) .&
             isfinite.(gn_meaninner_err) .&
             (gn_meaninner .> 0.0) .&
             (λs .>= 0.0) .&
             (λs .<= 1.0)
    ...
    mask_d = isfinite.(λspin) .&
             isfinite.(frac_unique1) .&
             (λspin .>= 0.15) .&
             (λspin .<= 0.5)

    mask_d2 = isfinite.(λspin) .&
              isfinite.(frac_opt_and_unique1) .&
              (λspin .>= 0.15) .&
              (λspin .<= 0.5)

    mask_d_inset = isfinite.(λspin) .&
    ...
    any(mask_d) || error("No usable entries for panel (d) main curve 1.")
    any(mask_d2) || error("No usable entries for panel (d) main curve 2.")
    any(mask_d_inset) || error("No usable entries for panel (d) inset.")
    ...
    errorbars!(axc,
        λs[mask_c], gn_meaninner[mask_c], gn_meaninner_err[mask_c];
        whiskerwidth = 0
    )

    scatterlines!(axc,
        λs[mask_c], gn_meaninner[mask_c];
        markersize = 8,
        marker = :circle
    )
    ...
    axd = styled_axis(bottom_right[1, 1];
        ...
        limits = ((0.14, 0.51), (-0.02, 1.02)),
    )

    p2 = scatterlines!(axd,
        λspin[mask_d2], frac_opt_and_unique1[mask_d2];
        color = "#E69F00",
        marker = :utriangle,
        markersize = 8,
        linewidth = 2,
    ...
    p1 = scatterlines!(axd,
        λspin[mask_d], frac_unique1[mask_d];
        color = "#0072B2",
        marker = :circle,
        markersize = 8,
        linewidth = 2
    ...
    axislegend(axd, [p1, p2],
        [L"N^*_{\mathrm{spin}}=1",
         L"r = 1,\; N^*_{\mathrm{spin}} = 1"];
        position = :rb,
        labelsize = 12,
        framevisible = false,
        valign = 0.05
    ...
    axd_inset = Axis(bottom_right[1, 1];
        width = Relative(0.46),
        height = Relative(0.46),
        halign = 0.95,
        valign = 0.94,
        ...
        xticks = (0.15:0.10:0.45, [@sprintf("%.2f", x) for x in 0.15:0.10:0.45]),
        xticklabelsize = INSET_TICKSIZE,
        yticklabelsize = INSET_TICKSIZE,
        xlabelsize = INSET_LABELSIZE,
        ylabelsize = INSET_LABELSIZE,
    ...
    scatterlines!(axd_inset,
        λspin[mask_d_inset], median_unique[mask_d_inset];
        marker = :circle,
        markersize = 6,
        linewidth = 1.5
    )
