# scripts/debug_chi1_lambda1_devtheta.jl
#
# Debug χ=1 at λ=1.0: why devTheta_abs_mean != π/4 ?
#
# Additions vs earlier version:
#   - graph sanity: ne expected, Wtot expected (75.0 for N=50,k=3,unweighted)
#   - forensic print for "bad" sites (|Z|<1): neighbors, incident weights, and local-field proxy hi = Σ w_ij * Zj
#
...
# ---------------------------
# Diagnostic run: χ=1, λ=1.0
# ---------------------------
function main()
    println("========================================================")
    println("  DEBUG χ=1 at λ=1.0 : why devTheta_abs_mean != π/4 ?")
    ...
    out_path = joinpath(save_dir,
        "debug_chi1_lambda1_N$(N)_k$(k)_graphs$(length(graph_seeds))_nsw$(nsweeps)_$(weighted ? "weighted" : "unweighted").json"
    )
    ...
    for (idx, gs) in enumerate(graph_seeds)
        ...
        # Ensure χ=1 actually held
        ld = linkdims(ψ)
        max_ld = isempty(ld) ? 1 : maximum(ld)
        @assert max_ld == 1 "Expected χ=1 product state, but max linkdim = $max_ld"
        ...
        if !isempty(bad)
            ...
            for i in bad
                ...
                hi = 0.0
                println("        incident weights and neighbor Z:")
                for j in nbrs
                    wij = wg.weights[i, j]
                    hi += wij * mz[j]
                    println("          j=$j  w_ij=$(wij)  Zj=$(mz[j])")
                end
    ...
    summary = Dict(
        "what" => "debug χ=1, λ=1.0 devTheta_abs vs π/4 (with graph/weight sanity + bad-site forensic)",
